...
export default class WorkspacesList extends React.PureComponent<Props, State> {
  ...
  private buildRows(): RowData[] {
    const { history, deletingId, workspaces } = this.props;
    const { filtered, selected, sortBy } = this.state;

    return buildRows(history, workspaces, deletingId, filtered, selected, sortBy);
  }
  ...
  private actionResolver(rowData: IRowData): IAction[] {
    ...
    return [
      ...
      },
      {
        ...
        isDisabled: !this.isEnabledAction(WorkspaceAction.DELETE_WORKSPACE, workspace),
      },
    ];
  }
  ...
  private isEnabledAction(action: WorkspaceAction, workspace: Workspace): boolean {
    ...
    return workspace.status !== DevWorkspaceStatus.TERMINATING;
  }
  ...
  private areActionsDisabled(rowData: IRowData): boolean {
    const id = (rowData as RowData).props.workspaceId;
    return this.props.deletingId.includes(id);
  }
  ...
  public componentDidUpdate(prevProps: Props): void {
    ...
    if (prevProps.deletingId.length !== this.props.deletingId.length) {
      const selected = this.state.selected.filter(id => !this.props.deletingId.includes(id));
      this.setState({
        selected,
      });
    }
